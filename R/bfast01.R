bfast01 <- function (data, formula = NULL, test = "OLS-MOSUM", level = 0.05, 
                     ...
                     stl = "none") 
{
  ...
  formula2 <- update(update(formula, . ~ segment/(.)), . ~ 
                       . - 1)
  ...
  improvement01 <- function(test) {
    ...
    if (test == "supLM") 
      return(sctest(gefp(formula, data = data), functional = supLM(trim01))$p.value < 
               level)
